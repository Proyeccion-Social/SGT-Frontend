...
-- ==========================================
-- SCRIPT: Tutor Activado con Disponibilidad 24/7
-- ==========================================
DO $$
DECLARE
    v_tutor_id uuid;
    ...
    v_time time;
BEGIN
    -- Obtener el ID del tutor usando su correo
    SELECT id_user INTO v_tutor_id
    FROM public.users
    WHERE email = 'tutor1@udistrital.edu.co';

    IF v_tutor_id IS NULL THEN
        RAISE EXCEPTION 'No se encontró el tutor con email tutor1@udistrital.edu.co';
    END IF;

    -- Actualizar contraseña y activar usuario
    UPDATE public.users 
    SET password = '$2b$10$8P3LKo9s/FX5dBlA/WzZsOEZQKyivEdr2Pd2ipIx73K0auL8Po9b2',
        status = 'ACTIVE',
        email_verified = true
    WHERE id_user = v_tutor_id;

    -- Asegurar que el tutor esté activo en la tabla tutors y tenga el límite máximo
    UPDATE public.tutors
    SET is_active = true,
        limit_disponibility = 168,
        profile_completed = true
    WHERE id_user = v_tutor_id;

    -- Generar disponibilidad de Lunes a Domingo, de 00:00 a 23:00
    FOR v_day IN 1..7 LOOP
        FOR v_hour IN 0..23 LOOP
            v_time := (v_hour::text || ':00:00')::time;
            
            -- Verificar si existe el slot en la tabla de disponibilidades general
            SELECT id_availability INTO v_avail_id
            FROM public.availability
            WHERE day_of_week = v_day AND start_time = v_time;

            -- Si no existe, crearlo
            IF v_avail_id IS NULL THEN
                INSERT INTO public.availability (day_of_week, start_time)
                VALUES (v_day, v_time)
                RETURNING id_availability INTO v_avail_id;
            END IF;

            -- Vincular el slot al tutor especificando la modalidad
            INSERT INTO public.tutor_have_availability (id_tutor, id_availability, modality)
            VALUES (v_tutor_id, v_avail_id, 'VIRT')
            ON CONFLICT (id_tutor, id_availability) DO NOTHING;
        END LOOP;
    END LOOP;
END $$;
