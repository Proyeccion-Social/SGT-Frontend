...
import { getSessionModifications, rejectModification } from '@features/sessions/services/sessionService';
...
export const PATCH: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (!requestId) {
      console.error('[BFF] reject-modification: no se encontró requestId para sessionId:', sessionId);
      return new Response(
    ...
    const data = await rejectModification(sessionId, requestId, token);
